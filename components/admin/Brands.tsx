...
import { Brand, LanguageCode } from '../../types';
...
interface BrandsProps {
  lang: LanguageCode;
  brands: Brand[];
  onAdd: (b: Omit<Brand, 'id'>) => void;
  onEdit: (b: Brand) => void;
  onDelete: (id: string) => void;
}

export const Brands: React.FC<BrandsProps> = ({ lang, brands, onAdd, onEdit, onDelete }) => {
  ...
  const filteredItems = brands.filter(b => b.name.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">{t.brandsDesc}</p>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
            ...
            <Button onClick={openAdd} className="whitespace-nowrap"><Plus size={18} /> {t.addBrand}</Button>
        </div>
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? t.editBrand : t.addBrand}>
