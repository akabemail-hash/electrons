...
import { Warehouse as WarehouseIcon, Search, Plus, Edit2, Trash2 } from 'lucide-react';
import { Warehouse, LanguageCode } from '../../types';
...
export const Warehouses: React.FC<WarehousesProps> = ({ lang, warehouses, onAdd, onEdit, onDelete }) => {
  ...
  const openEdit = (item: Warehouse) => {
    setEditingItem(item);
    setForm({ name: item.name, location: item.location, status: item.status });
    setIsModalOpen(true);
  };
  ...
  const filteredItems = warehouses.filter(w => w.name.toLowerCase().includes(searchTerm.toLowerCase()));
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          <h2 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <WarehouseIcon className="text-primary-500" />
            {t.warehouses}
          </h2>
        </div>
        <div className="flex gap-2 w-full md:w-auto">
            ...
            <Button onClick={openAdd} className="whitespace-nowrap"><Plus size={18} /> {t.addWarehouse || "Add Warehouse"}</Button>
        </div>
      </div>
